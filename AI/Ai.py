...
def ask_gemini(code):
    """ Sends the code to Gemini API and formats the feedback """
    response = chat_session.send_message(f"Review this Python code and provide structured feedback:\n\n{code}")
    feedback = response.text

    # Formatting Gemini's response into readable sections
    feedback = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', feedback)  # Bold text
    feedback = re.sub(r'\*(.*?)\*', r'<em>\1</em>', feedback)  # Italics
    feedback = re.sub(r'(\n\s*-)', r'<br><ul>\1', feedback)  # Bullet points
    feedback = re.sub(r'(-\s+)', r'<li>', feedback)  # List items
    feedback = feedback.replace('</ul>', '') + '</ul>'  # Close list properly
